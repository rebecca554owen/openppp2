...
int main()
{
    ...
    // The Noise RecordProtector exporter already bound ivv, carrier,
    // canonical key id and transcript into the 32-byte record root; the HKDF
    // layer only separates direction/purpose.
    {
        std::array<uint8_t, 32> record_root = {};
        for (int i = 0; i < 32; ++i) {
            record_root[i] = static_cast<uint8_t>(i + 1);
        }

        RecordKeyMaterial m;
        if (!DeriveRecordKeyMaterial(record_root.data(), record_root.size(), m)) {
        ...
        if (!DeriveRecordKeyMaterial(record_root.data(), record_root.size(), m2)) {
        ...
        AuthenticatedRecordProtector c2s(
            ...
            RecordDirection::ClientToServer, 0, "aes-256-gcm");
        ...
        AuthenticatedRecordProtector s2c(
            ...
            RecordDirection::ServerToClient, 0, "aes-256-gcm");
