...
int PppApplication::PreparedArgumentEnvironment(int argc, const char* argv[]) noexcept
{
    ...
    IAsynchronousWriteIoQueue::SafeMode(ppp::ToBoolean(ppp::GetCommandArgument("--tun-sq", argc, argv, "y").data())); /* SSQ. */
...
#if defined(_WIN32)
    return ppp::win32::Win32Native::AddShutdownApplicationEventHandler(callbackf);
#else
    return ppp::unix__::UnixAfx::AddShutdownApplicationEventHandler(callbackf);
#endif
}
