...
bool PppApplication::PreparedLoopbackEnvironment(const std::shared_ptr<NetworkInterface>& network_interface) noexcept
{
    ...
    if (client_mode_)
    {
        ...
        do
        {
            ...
            ethernet->MuxFast(&network_interface->MuxFast);
    ...
    else
    {
        ...
        do
        {
            ...
            ethernet = ppp::make_shared_object<VirtualEthernetSwitcher>(configuration, network_interface->MuxFast);
            if (NULL == ethernet)
            {
                break;
            }
...
std::shared_ptr<NetworkInterface> PppApplication::GetNetworkInterface(int argc, const char* argv[]) noexcept
{
    ...
    if (NULL != ni)
    {
        ...
        ni->MuxFast = ppp::ToBoolean(ppp::GetCommandArgument("--tun-mux-fast", argc, argv).data());
