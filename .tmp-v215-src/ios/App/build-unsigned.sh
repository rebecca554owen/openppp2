...
if [ -z "$OPENSSL_LIB_DIR" ]; then
	printf 'OPENSSL_LIB_DIR is required and must point to iOS %s OpenSSL libraries.\n' "$PRODUCT_SDK" >&2
	exit 2
fi
...
# Avoid stale host-app Swift modules shadowing the OpenPPP2 C bridge module.
...
if [ -n "$OPENSSL_LIB_DIR" ]; then
	set -- "$@" "OPENSSL_LIB_DIR=$OPENSSL_LIB_DIR"
fi
...
APP_BUNDLE="$APP_BUNDLE" IPA_NAME="OpenPPP2-$PRODUCT_SDK" "$ROOT_DIR/ios/package-ipa.sh"
