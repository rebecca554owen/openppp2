    ...
    namespace win32
    {
        namespace network
        {
            ...
            static bool FixGatewayServerAddress(const ppp::win32::network::AdapterInterfacePtr& ai) noexcept
            {
                ...
                boost::asio::ip::address nx = boost::asio::ip::address::from_string(ai->GatewayServer.data(), ec);
                ...
                boost::asio::ip::address ip = boost::asio::ip::address::from_string(ai->Address.data(), ec);
                if (ec)
                {
                    return false;
                }

                boost::asio::ip::address mask = boost::asio::ip::address::from_string(ai->Mask.data(), ec);
                if (ec)
                {
                    return false;
                }
            ...
            static ppp::win32::network::AdapterInterfacePtr SelectBaseNetowrkInterface(std::vector<ppp::win32::network::AdapterInterfacePtr>& adapters, bool fix) noexcept
            {
                ...
                else
                {
                    ...
                    {
                        boost::system::error_code ec;
                        boost::asio::ip::address gw = boost::asio::ip::address::from_string(ai->GatewayServer.data(), ec);
                        if (ec)
                        {
                            continue;
                        }
            ...
            AdapterInterfacePtr GetUnderlyingNetowrkInterface(const ppp::string& id) noexcept
            {
                ...
                {
                    int preferred_index = GetNetworkInterfaceIndexByDefaultRoute();
                    if (preferred_index != -1)
                    {
                        for (auto&& ai : adapters)
                        {
                            if (ai->IfIndex == preferred_index)
                            {
                                if (FixGatewayServerAddress(ai))
                                {
                                    return ai;
            ...
            int SetAllNicsDnsAddresses(ppp::vector<boost::asio::ip::address>& servers, ppp::unordered_map<int, ppp::unordered_set<boost::asio::ip::address>>& addresses) noexcept
            {
                ...
                for (NetworkInterfacePtr& ni : interfaces)
                {
                    ...
                    {
                        boost::system::error_code ec;
                        boost::asio::ip::address ip = boost::asio::ip::address::from_string(ips.data(), ec);
                        if (ec)
                        {
                            continue;
                        }
