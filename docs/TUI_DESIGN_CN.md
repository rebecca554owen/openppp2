...
| 命令                  | 别名          | 动作 |
|-----------------------|---------------|------|
| `openppp2 help`       | `help`        | 向命令输出区段打印命令列表 |
...
| `openppp2 info`       | `status`      | 将当前信息快照复制到命令输出区 |
| `openppp2 clear`      | `clear`       | 清空命令输出环形缓冲区 |
...
**真实终端光标在 TUI 会话的全生命周期内始终隐藏**，只有 `Stop()` 在退出时才会将其恢复可见。输入行中的插入位置通过一个反显白色字符块来指示，该字符块由 `BuildEditorLine()` 按字节位置嵌入：
...
| `void UpdateStatus(string status)` | 推送状态栏文本（VPN 状态 + 速度），线程安全 |
...
| ErrorCode | 说明 |
|-----------|------|
| `TuiStartFailed` | ConsoleUI::Start() 初始化失败 |
| `TuiRenderThreadFailed` | 渲染线程启动失败 |
| `TuiInputThreadFailed` | 输入线程启动失败 |
| `TuiTerminalSetupFailed` | 终端原始模式配置失败 |
| `TuiVtProcessingFailed` | VT100 处理启用失败（Windows 专属） |
