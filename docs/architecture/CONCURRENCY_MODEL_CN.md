...
> **上一层索引：**[返回索引](README_CN.md) · **English：**[Concurrency Model](CONCURRENCY_MODEL.md)
...
`RouteState::Snapshot()` 在互斥锁内复制容器，不返回可变容器指针。`RouteCoordinator::Stop()` 使用原子状态保证幂等；平台回滚数据封装在各自 `IRoutePlatform` 快照中。

DNS 异步任务复制 `shared_ptr<const DnsSessionContext>`。Context 使用原子 active 标志并弱引用 `IDnsTunnelTransport`，因此关闭 Controller 会阻止后续发送，但不会延长 Exchanger 生命周期。`DnsController::Close()` 幂等，并且先于 Exchanger 释放执行。
...
- [`LINKLAYER_PROTOCOL_CN.md`](../reference/LINKLAYER_PROTOCOL_CN.md) — 隧道动作词汇表
