...
在 OPENPPP2 中，包格式不是单纯的序列化细节。它本身就是安全模型和运行模型的一部分，因为它直接决定：
...
这是一个很紧凑的字段复用技巧：一个字段同时传达了标识和协议类别。
...
这些 pseudo 地址字段用来承载虚拟包语义中的源/目的端点信息。对于 UDP 包，解包器还会检查这些地址和端口是否符合 UDP 语义。

因此 static packet 不是 opaque blob，而是一种带结构化网络语义的封装格式。
...
1. 检查输入
2. 按 session identity 解析对应 cipher
3. 如果存在 transport cipher，先加密 payload
...
9. 混淆 `session_id`
10. 计算 checksum
11. 如果配置了 protocol cipher，则对 trailing header body 加密
12. 对 `session_id` 及之后整段做 shuffle
13. 对 `session_id` 及之后整段做 masking
14. 最后整体 delta encode
...
5. 从 `session_id` 起整段 unmask
6. 从 `session_id` 起整段 unshuffle
...
顺序不能乱。顺序一旦颠倒，包就无法通过结构校验。

## static 模式里的动态头长行为

一个关键实现细节是：如果 protocol cipher 对 trailing header body 的加密或解密导致这一段长度发生变化，代码会重新拼出整个包缓冲区，并更新 `header_length`。

这说明：

- static packet path 不是死板地假定 ciphertext 扩展固定不变
- `header_length` 是真正参与格式语义的动态字段，而不是装饰值

## static packet 的 session-ciphertext 派生

`VirtualEthernetPacket::Ciphertext(...)` 会用以下几个量拼出派生字符串：
...
这说明 static packet 的保护同样是 session-shaped、identity-shaped 的，而不是整个运行时所有 static 包都完全一样。
...
## 为什么这些包格式绝不能简化成“头 + 密文 payload”

不管是常规 transmission 帧族还是 static packet 帧族，都不应粗糙地写成“一个头，再加一个加密 payload”。因为实际上，元数据本身也在被保护和扰动：
...
这就是为什么包格式文档在 OPENPPP2 里是核心材料，而不是附属材料。
