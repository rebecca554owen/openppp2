...
> **上一层索引：**[返回索引](README_CN.md) · **English：**[Security Model](SECURITY.md)
...
| 参数 | 配置键 | 用途 |
|------|--------|------|
| 协议密钥 | `key.protocol-key` | 协议层密码基础密钥 |
| 传输密钥 | `key.transport-key` | 传输层密码基础密钥 |
...
sequenceDiagram
    ...
    C->>S: ivv（Int128 随机值，连接级密钥变化种子）
    S->>C: nmux（多路复用参数，低位 = mux 使能标志）

    Note over C,S: 双方均设置 handshaked_ = true
    Note over C,S: 使用 protocol-key + ivv_str / transport-key + ivv_str 重建 protocol_ 和 transport_ cipher
...
代码**能**证明：
- 存在会话级工作 cipher 状态重建并被应用（`base_key + ivv_str`）。
...
| ErrorCode | 描述 | 来源 |
|-----------|------|------|
| `SessionHandshakeFailed` | 握手序列执行失败 | `ITransmission::HandshakeClient/Server` |
| `SessionHandshakeFailed` | 握手超过配置的超时时间 | `ITransmission` 超时逻辑 |
| `SessionAuthFailed` | 被服务端或后端拒绝认证 | `VirtualEthernetSwitcher` |
| `EvpInitKeyDerivationFailed` | 重建工作 cipher 状态时 cipher/KDF 初始化失败 | cipher 初始化 |
| `VEthernetManagedAuthNullCallback` | 后端拒绝用户认证 | `VirtualEthernetManagedServer` |
| `SessionQuotaExceeded` | 用户额度耗尽 | `VirtualEthernetManagedServer` |
| `AuthTokenExpired` | 用户订阅已过期 | `VirtualEthernetManagedServer` |
| `SslHandshakeFailed` | TLS 协商失败（WSS 承载） | `ISslWebsocketTransmission` |
| `SessionDisposed` | 会话已被释放 | Dispose 模式 |
| `KeepaliveTimeout` | 心跳超时 | `DoKeepAlived` |

> **注**：密钥派生和 TLS 协商失败等旧设计名不是当前 `ErrorCodes.def` 条目；请分别使用 `EvpInitKeyDerivationFailed` 和 `SslHandshakeFailed`。
...
**配置安全：**
- [ ] 预共享密钥（`key.protocol-key`、`key.transport-key`）每次部署唯一，并以文件系统级访问控制存储（`chmod 600 appsettings.json`）。
...
```json
{
    "key": {
        ...
        "protocol-key": "strong-random-protocol-secret-at-least-32-chars",
        "transport": "aes-256-cfb",
        "transport-key": "strong-random-transport-secret-at-least-32-chars",
        "kf": 154543927,
        "kx": 128,
        "kl": 10,
        "kh": 12,
...
- [`TRANSMISSION_CN.md`](../architecture/TRANSMISSION_CN.md) — cipher 槽位细节、帧化、握手序列
- [`HANDSHAKE_SEQUENCE_CN.md`](../architecture/HANDSHAKE_SEQUENCE_CN.md) — 逐步握手时序
- [`ARCHITECTURE_CN.md`](../architecture/ARCHITECTURE_CN.md) — 系统架构与组件信任边界
- [`TUNNEL_DESIGN_CN.md`](../architecture/TUNNEL_DESIGN_CN.md) — 逐层隧道设计
- [`ROUTING_AND_DNS_CN.md`](../guides/ROUTING_AND_DNS_CN.md) — 路由与 DNS 安全控制
- [`MANAGEMENT_BACKEND_CN.md`](../guides/MANAGEMENT_BACKEND_CN.md) — 后端认证与额度执行
- [`EDSM_STATE_MACHINES_CN.md`](../architecture/EDSM_STATE_MACHINES_CN.md) — 会话生命周期与 dispose 模式
