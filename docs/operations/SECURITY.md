...
> **Parent index:** [Back to index](README.md) · **Chinese:** [安全模型](SECURITY_CN.md)
...
```
Working key = Cipher(base_key + ivv_str)

protocol_working_key  = Cipher(key.protocol-key  + ivv_str)
transport_working_key = Cipher(key.transport-key + ivv_str)
```

The `ivv` is an `Int128` random value generated by the client during handshake. Each connection gets a different `ivv`, so working keys differ across connections even when the base key is identical.
...
|-----------|------------|---------|
| Protocol key | `key.protocol-key` | Protocol-layer cipher base secret |
| Transport key | `key.transport-key` | Transport-layer cipher base secret |
...
sequenceDiagram
    ...
    C->>S: ivv (Int128 random value, per-connection key variation seed)
    S->>C: nmux (mux parameter; low bit = mux enable flag)

    Note over C,S: Both sides set handshaked_ = true
    Note over C,S: Rebuild protocol_ and transport_ ciphers using protocol-key + ivv_str / transport-key + ivv_str
...
|-----------|-------------|-----------------|
| `SessionHandshakeFailed` | Handshake sequence execution failed | `ITransmission::HandshakeClient/Server` |
| `SessionHandshakeFailed` | Handshake exceeded configured timeout | `ITransmission` timeout logic |
| `SessionAuthFailed` | Authentication rejected by server or backend | `VirtualEthernetSwitcher` |
| `EvpInitKeyDerivationFailed` | Cipher/KDF initialization failed while rebuilding working cipher state | Cipher initialization |
| `VEthernetManagedAuthNullCallback` | Backend rejected user authentication | `VirtualEthernetManagedServer` |
| `SessionQuotaExceeded` | User quota exhausted | `VirtualEthernetManagedServer` |
| `AuthTokenExpired` | User subscription expired | `VirtualEthernetManagedServer` |
| `SslHandshakeFailed` | TLS negotiation failed (WSS carrier) | `ISslWebsocketTransmission` |
| `SessionDisposed` | Session was already disposed | Dispose pattern |
| `KeepaliveTimeout` | Keepalive heartbeat timed out | `DoKeepAlived` |

> **Note**: Older design-only names for key-derivation and TLS negotiation failures are not current `ErrorCodes.def` entries; use `EvpInitKeyDerivationFailed` and `SslHandshakeFailed` respectively.
...
- [ ] Pre-shared keys (`key.protocol-key`, `key.transport-key`) are unique per deployment and stored with filesystem-level access control (`chmod 600 appsettings.json`).
...
- [`TRANSMISSION.md`](../architecture/TRANSMISSION.md) — Cipher slot details, framing, handshake sequence
- [`HANDSHAKE_SEQUENCE.md`](../architecture/HANDSHAKE_SEQUENCE.md) — Step-by-step handshake timing
- [`ARCHITECTURE.md`](../architecture/ARCHITECTURE.md) — System architecture and component trust boundaries
- [`TUNNEL_DESIGN.md`](../architecture/TUNNEL_DESIGN.md) — Layer-by-layer tunnel design
- [`ROUTING_AND_DNS.md`](../guides/ROUTING_AND_DNS.md) — Route and DNS security controls
- [`MANAGEMENT_BACKEND.md`](../guides/MANAGEMENT_BACKEND.md) — Backend authentication and quota enforcement
- [`EDSM_STATE_MACHINES.md`](../architecture/EDSM_STATE_MACHINES.md) — Session lifecycle and dispose pattern
