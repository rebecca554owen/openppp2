...
sequenceDiagram
    ...
    note over C,S: 双方：根据 base key + ivv 重建 protocol_ 和 transport_
...
flowchart TD
    ...
    G --> H["mux = nmux & 1"]
    H --> I["根据 base key + ivv 重建 protocol_ 和 transport_"]
    I --> J[握手完成]
```
...
此时双方拥有相同的 `ivv` 和 `nmux`，能够从 `base_key + ivv + nmux` 派生出相同的
工作密码。
...
flowchart TD
    ...
    B -->|"所有步骤成功"| SUCCESS
    FAIL["SetLastErrorCode + 返回 false\nTransmission 被销毁"]
...
2. **传输密码**（外层）— 以握手结果为密钥。具体来说，客户端的 `ivv`、服务端的
   `nmux` 以及基础密钥材料共同产生传输密码。这个密码是连接级的，在握手完成时由
   cipher 重建步骤激活。
