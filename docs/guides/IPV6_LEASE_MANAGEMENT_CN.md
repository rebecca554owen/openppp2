...
> **上一层索引：**[返回索引](README_CN.md) · **English：**[IPv6 Lease Management](IPV6_LEASE_MANAGEMENT.md)


[English Version](IPV6_LEASE_MANAGEMENT.md)
...
### 2.1 `IPv6LeaseEntry`（`VirtualEthernetSwitcher.h`，第 130 行）
...
### 2.4 `IPv6RequestEntry`（`VirtualEthernetSwitcher.h`，第 109 行）
...
| `IPv6LeaseConflict` | `kError` | 所有 `kMaxIPv6AllocRetries` 个候选地址均被拒绝；池已满 |
| `IPv6LeaseCommitFailed` | `kError` | 最终重试后 `try_commit_ipv6_lease` 返回 false |
| `IPv6LeaseConflict` | `kError` | 新会话试图申请已被不同活跃会话持有的地址 |
| `IPv6AddressConflict` | `kError` | 分配的地址与现有活跃租约冲突 |
| `IPv6StaticBindingFailed` | `kError` | 来自配置的静态绑定无法提交 |

> **注**：以下错误码为拟新增/设计项，不在当前 `ErrorCodes.def`：`IPv6LeaseCommitFailed`（近似现有码 `IPv6LeaseConflict`）、`IPv6AddressConflict`（近似 `IPv6LeaseConflict`）、`IPv6StaticBindingFailed`（无近似现有码，需新增）。
