...
1. 根目录包含示例 RSA 私钥/证书和示例配置；经维护者确认，这些文件按示例资产处理，可以随 release 分发，不作为 release 阻断项。建议启动时检测示例 key/cert 时输出醒目 warning（已部分实施，见 §3.5）。
...
6. 修复主要 Dispose/Finalize one-shot。→ **⚠️ 核心传输/写队列/Exchanger/WebSocket/socket_ 已修复，全仓治理仍需分级复核（见 §15）。**
...
**核心建议：** 先合入当前未提交修复（Timer.cpp / DnsResolver.cpp / SSL.cpp / ITransmission.{h,cpp} / VEthernetNetworkSwitcher.cpp / android/CMakeLists.txt），再按 §14.11 优先级 1 的三项依次落地，可在不破坏已有 macOS/Android 修复的前提下补齐安全与性能短板。同时持续推进首版 §8 的 P0/P1 项（写队列背压、传输帧上限）。
...
| 组件 | 文件 | `disposed_` 类型 | 风险说明 | 优先级 |
|---|---|---|---|---|
| `VEthernetExchanger`（client） | `ppp/app/client/VEthernetExchanger.h:1029` | `bool disposed_:1`（bitfield） | bitfield 无法原子化；跨 strand 回调中大量 `if (disposed_)` 读取；`while (!disposed_)` 循环；Finalize 与异步回调竞争 | **P0** |

> **注意：** `bool disposed_:1` 是 bitfield，不能直接用 `std::atomic` 包装。修复方案需要将 bitfield 改为独立的 `std::atomic_bool` 成员，并实现 `exchange` one-shot。这涉及 `VEthernetExchanger`（client）的 Finalize 和所有异步回调路径，需要仔细验证 strand 语义。
...
当前已修复的组件覆盖了**核心传输路径**（ITransmission、IAsynchronousWriteIoQueue）、**服务端交换器**（VirtualEthernetExchanger server）、**WebSocket 传输层**和 **socket_ shared_ptr 并发保护**。

**全仓 Dispose/Finalize 治理尚未完成**，仍需：

1. **P0**：修复 `VEthernetExchanger`（client）的 bitfield `disposed_`，改为 `std::atomic_bool` + `exchange` one-shot。
