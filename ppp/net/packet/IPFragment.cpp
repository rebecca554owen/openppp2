    ...
    namespace net {
        namespace packet {
            ...
            bool IPFragment::Input(const std::shared_ptr<IPFrame>& packet) noexcept {
                if (NULLPTR == packet) {
                    ppp::diagnostics::SetLastErrorCode(ppp::diagnostics::ErrorCode::IpFragmentInputNullPacket);
                    return false;
                }
                ...
                if ((packet->Flags & IPFlags::IP_MF) != 0 || ((packet->Flags & IPFlags::IP_OFFMASK) != 0 && packet->GetFragmentOffset() > 0)) {
                    ...
                    if (NULLPTR == payload || 0 >= payload->Length) {
                    ...
                    do {
                        ...
                        bool completed = false;
                        {
                            ...
                            if (tail != endl) {
                                ...
                                if (NULLPTR == subpackage) {
                                    ppp::diagnostics::SetLastErrorCode(ppp::diagnostics::ErrorCode::IpFragmentSubpackageAllocFailed);
                                    return false;
                                }
                        ...
                        if (completed) {
                            ...
                            if (NULLPTR == buffer) {
                                ppp::diagnostics::SetLastErrorCode(ppp::diagnostics::ErrorCode::IpFragmentReassemblyBufferAllocFailed);
                                return false;
                            }
                            ...
                            for (size_t index = 0, count = frames.size(); index < count; index++) {
                                ...
                                if (NULLPTR == payload || NULLPTR == payload->Buffer || 0 >= payload->Length) {
                                    ppp::diagnostics::SetLastErrorCode(ppp::diagnostics::ErrorCode::NetworkPacketMalformed);
                                    return false;
                                }

                                if (false == ms.Write(payload->Buffer.get(), 0, payload->Length)) {
                            ...
                            if (NULLPTR == originNew) {
                                ppp::diagnostics::SetLastErrorCode(ppp::diagnostics::ErrorCode::IpFragmentReassemblyFrameAllocFailed);
                                return false;
                            }
                            ...
                            if (NULLPTR == packet_payload) {
                                ppp::diagnostics::SetLastErrorCode(ppp::diagnostics::ErrorCode::IpFragmentReassemblyPayloadAllocFailed);
                                return false;
                            }
            ...
            bool IPFragment::Output(const IPFrame* packet) noexcept {
                ...
                if (NULLPTR == frame) {
                    ppp::diagnostics::SetLastErrorCode(ppp::diagnostics::ErrorCode::IpFragmentOutputNullPacket);
                    return false;
                }
                ...
                for (int i = 0; i < subpacketl; i++) {
                    ...
                    if (NULLPTR == message_ || 0 >= message_->Length) {
                        ppp::diagnostics::SetLastErrorCode(ppp::diagnostics::ErrorCode::ProtocolEncodeFailed);
                        return false;
                    }
