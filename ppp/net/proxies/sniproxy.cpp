    ...
    namespace net {
        namespace proxies {
            ...
            bool sniproxy::do_connect_and_forward_to_host(const boost::asio::yield_context& y, const ppp::string hostname_, int self_websocket_port, int forward_connect_port, MemoryStream& messages_) noexcept {
                ...
                if (be_host(configuration_->websocket.host, hostname_)) {
                    ...
                    if (configuration_->cdn[0] == forward_connect_port || configuration_->cdn[1] == forward_connect_port) {
                        boost::asio::ip::address interfaceIP_ = boost::asio::ip::address::from_string(configuration_->ip.interface_.data(), ec_);
                        boost::asio::ip::address publicIP_ = boost::asio::ip::address::from_string(configuration_->ip.public_.data(), ec_);
            ...
            bool sniproxy::handshake() noexcept {
                ...
                timeout_ = Timer::Timeout((uint64_t)configuration_->tcp.connect.timeout * 1000, make_shared_object<Timer::TimeoutEventHandler>(
                    [this, self]() noexcept {
                        close();
                    }));
                if (!timeout_) {
