...
namespace ppp {
    namespace io {
...
#else
            int status = ::unlink(fullpath.data());
            return status == 0;
#endif
        }
        ...
        static bool FILE_GetAllFileNames(const char* path, ppp::vector<std::string>& out) noexcept {
            if (NULL == path || *path == '\x0') {
                return false;
            }
        ...
        bool File::GetAllFileNames(const char* path, bool recursion, ppp::vector<std::string>& out) noexcept {
            if (recursion) {
