 ...
 *
 * Thread safety: the send and receive sequence counters are allocated with
 * atomic compare-and-swap, so Seal() never hands the same sequence (and thus
 * the same GCM nonce) to two concurrent writers, and Open() advances the
 * receive counter exactly once per authenticated sequence even when records
 * are delivered concurrently.  A failed Seal() may leave a gap in the send
 * stream but never reuses a nonce; a failed Open() never advances the receive
 * counter.
 */
