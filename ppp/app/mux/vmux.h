...
#define vmux_spawn(context_ptr, strand_ptr, fx) \
    ...
    bool                                                                        vmux_post_exec(const TContextPtr& context, const TStrandPtr& strand, LegacyCompletionHandler&& handler) noexcept {
        ...
        if (strand_copy) {
            ...
            boost::asio::dispatch(*strand_copy, invoked);
            return true;
        }
        ...
        if (context_copy) {
            ...
            boost::asio::dispatch(*context_copy, invoked);
            return true;
        }
        else {
            return false;
    ...
    boost::asio::ip::basic_endpoint<TProtocol>                                  vmux_get_address_by_host_name_internal(TIterator& i, TIterator& l, int port) noexcept {
        return ppp::net::asio::internal::GetAddressByHostName(i, l, port);
    }

    template <class TProtocol, class ResolveCall>
    boost::asio::ip::basic_endpoint<TProtocol>                                  vmux_get_address_by_host_name_internal(boost::asio::ip::basic_resolver<TProtocol>& resolver, const char* hostname, int port, ResolveCall&& resolver_resolve) noexcept {
        return ppp::net::asio::internal::GetAddressByHostName(resolver, hostname, port, resolver_resolve);
    }

    template <class TProtocol>
    boost::asio::ip::basic_endpoint<TProtocol>                                  vmux_get_address_by_host_name(boost::asio::ip::basic_resolver<TProtocol>& resolver, const char* hostname, int port) noexcept {
        return ppp::net::asio::GetAddressByHostName(resolver, hostname, port);
    }

    template <class TProtocol>
    boost::asio::ip::basic_endpoint<TProtocol>                                  vmux_get_address_by_host_name(boost::asio::ip::basic_resolver<TProtocol>& resolver, const char* hostname, int port, const boost::asio::yield_context& y) noexcept {
        return ppp::net::asio::GetAddressByHostName(resolver, hostname, port, y);
    }
}