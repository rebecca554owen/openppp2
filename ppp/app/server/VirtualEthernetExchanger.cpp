...
namespace ppp {
    namespace app {
        namespace server {
            ...
            bool VirtualEthernetExchanger::OnMux(const ITransmissionPtr& transmission, uint16_t vlan, uint16_t max_connections, YieldContext& y) noexcept {
                bool err = true;
                for (;;) {
                    if (disposed_) {
                        break;
                    ...
                    mux = make_shared_object<vmux::vmux_net>(vmux_context, vmux_strand, max_connections, 
                        switcher_->mux_fast_ ? vmux::vmux_net::vmux_fast_mode_must : vmux::vmux_net::vmux_fast_mode_auto, true);
                ...
                if (err) {
                    ...
                    DoMux(transmission, 0, 0, y);
                }
                else {
                    DoMux(transmission, vlan, max_connections, y);
                }
            ...
            bool VirtualEthernetExchanger::INTERNAL_RedirectDnsQuery(
                ...
                const Ipep::GetAddressByHostNameCallback cb = 
                    [self, this, buffer, packet_length, static_transit, source, in, destination](IPEndPoint* redirectEP) noexcept {
                        if (!redirectEP) {
                            return false;
                        }
