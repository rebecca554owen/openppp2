        ...
        namespace server {
            ...
            bool VirtualEthernetExchanger::Arp(const ITransmissionPtr& transmission, uint32_t ip, uint32_t mask) noexcept {
                ...
                if (VirtualEthernetLoggerPtr logger = switcher_->GetLogger(); NULL != logger) {
                    logger->Arp(GetId(), transmission->GetRemoteEndPoint(), ip, mask);
                }
                return true;
            }
            ...
            bool VirtualEthernetExchanger::SendPacketToDestination(const ITransmissionPtr& transmission, 
                ...
                if (destinationPort == PPP_DNS_SYS_PORT) {
                    ...
                    if (hostDomain.size() > 0) {
                        if (NULL != logger) {
                            logger->Dns(GetId(), transmission->GetRemoteEndPoint(), hostDomain);
                        }
                ...
                if (NULL != datagram) {
                    ...
                    if (NULL != datagram) {
                        ...
                        if (ok) {
                            if (NULL != logger) {
                                logger->Port(GetId(), transmission->GetRemoteEndPoint(), datagram->GetSourceEndPoint(), datagram->GetLocalEndPoint());
                            }
            ...
            bool VirtualEthernetExchanger::INTERNAL_RedirectDnsQuery(
                ...
                Ipep::GetAddressByHostName(GetUResolver(), configuration->udp.dns.redirect, PPP_DNS_SYS_PORT, cb);
                return true;
            }
            ...
            bool VirtualEthernetExchanger::UploadTrafficToManagedServer() noexcept {
                ...
                ITransmissionStatisticsPtr statistics = transmission_->Statistics;
                if (NULL == statistics) {
                    return false;
                }
