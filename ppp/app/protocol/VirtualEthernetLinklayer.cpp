...
#pragma pack(pop)
            ...
            bool VirtualEthernetLinklayer::PacketInput(const ITransmissionPtr& transmission, Byte* p, int packet_length, YieldContext& y) noexcept {
                ...
                if (packet_action == PacketAction_PSH) {
                    ...
                    if (packet_length >= sizeof(VirtualEthernetLinklayer_MUX_IL) - 1) {
                        VirtualEthernetLinklayer_MUX_IL* pil = (VirtualEthernetLinklayer_MUX_IL*)(p - 1);
                        return OnMux(transmission, ntohs(pil->vlan), ntohs(pil->max_connections), y);
                    }
                }
            ...
            bool VirtualEthernetLinklayer::DoMux(const ITransmissionPtr& transmission, uint16_t vlan, uint16_t max_connections, YieldContext& y) noexcept {
                MemoryStream ms;
