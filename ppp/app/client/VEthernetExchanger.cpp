        ...
        namespace client {
                ...
                while (!disposed_) {
                    ...
                    if (NULL == vmux_context) {
                        ...
                        mux = make_shared_object<vmux::vmux_net>(vmux_context, vmux_strand, max_connections, 
                            switcher_->mux_fast_ ? vmux::vmux_net::vmux_fast_mode_must : vmux::vmux_net::vmux_fast_mode_none, false);
                    ...
                    successes = YieldContext::Spawn(buffer_allocator.get(), *vnet_context, vnet_strand.get(), 
                        [this, vnet_transmission, mux](YieldContext& y) noexcept {
                            bool ok = false;
                            if (!disposed_) {
                                uint16_t max_connections = mux->get_max_connections();
                                ok = DoMux(vnet_transmission, mux->Vlan, max_connections, y);
                            }
            ...
            bool VEthernetExchanger::MuxConnectAllLinklayers(const std::shared_ptr<ppp::threading::BufferswapAllocator>& allocator, const std::shared_ptr<vmux::vmux_net>& mux) noexcept {
                ...
                return YieldContext::Spawn(allocator.get(), *context, strand.get(),
                    [self, this, mux](YieldContext& y) noexcept -> bool {
            ...
            bool VEthernetExchanger::OnMux(const ITransmissionPtr& transmission, uint16_t vlan, uint16_t max_connections, YieldContext& y) noexcept {
                std::shared_ptr<vmux::vmux_net> mux = mux_;
                if (NULL != mux) {
