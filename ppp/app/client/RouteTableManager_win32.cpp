...
namespace ppp {
    namespace app {
        namespace client {
            ...
            RouteTableManager::NewWindowsRoutePlatform(const route::RoutePlanInput& input) noexcept {
            ...
            void RouteTableManager::AddRoute(const route::RoutePlanInput& input) noexcept {
                ...
                std::unique_ptr<route::WindowsRoutePlatform> platform = NewWindowsRoutePlatform(input);
                if (NULLPTR == platform) {
                    return;
                }
            ...
            bool RouteTableManager::DeleteRoute(const route::RoutePlanInput& input, const std::shared_ptr<MIB_IPFORWARDTABLE>& mib, uint32_t ip, uint32_t gw, int prefix) noexcept {
                (void)mib;
                std::unique_ptr<route::WindowsRoutePlatform> platform = NewWindowsRoutePlatform(input);
            ...
            void RouteTableManager::DeleteRouteWithDnsServers(const route::RoutePlanInput& input) noexcept {
                ...
                if (auto mib = ppp::win32::network::Router::GetIpForwardTable(); NULLPTR != mib) {
                    for (uint32_t ip : snapshot.dns_servers[0]) {
                        DeleteRoute(input, mib, ip, input.tap_gateway, 32);
                    }
                }
                ...
                if (gw.is_v4()) {
                    ...
                    if (auto mib = ppp::win32::network::Router::GetIpForwardTable(); NULLPTR != mib) {
                        for (uint32_t ip : snapshot.dns_servers[1]) {
                            DeleteRoute(input, mib, ip, next_hop, 32);
                        }
                    }
                }
            ...
            bool RouteTableManager::ProtectDefaultRoute(const route::RoutePlanInput& input) noexcept {
                ...
                state->remove_defaults = [input]() noexcept {
                    auto platform = NewWindowsRoutePlatform(input);
