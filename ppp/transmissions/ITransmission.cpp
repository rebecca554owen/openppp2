...
namespace ppp {
    namespace transmissions {
        ...
        // -----------------------------------------------------------------------------
        static constexpr int EVP_HEADER_TSS = 3;                  // Encrypted length field size (3 bytes, 24-bit)
        // Legacy packet header total size: seed byte + 3-byte length field.
        // Kept separate from EVP_HEADER_MSS so the base94 handshake wire
        // format (4-byte simple header + 3-byte extended checksum) stays
        // byte-identical across versions; only the post-handshake legacy
        // data frames grow from 3 to 4 header bytes.
        static constexpr int EVP_LEGACY_HEADER_LEN = EVP_HEADER_TSS + 1; // 4 bytes: seed + 24-bit length
        static constexpr int EVP_HEADER_MSS = 3;                  // base94 extended checksum size (3 bytes, wire-fixed)
...
#pragma GCC pop_options
        ...
        static std::shared_ptr<Byte> Transmission_Header_Encrypt(
            ...
            // Header array: [seed_byte, high_byte, mid_byte, low_byte]
            Byte EVP_payload_length_array[EVP_LEGACY_HEADER_LEN] = {
                (Byte)(RandomNext(0x01, 0xff)),
                (Byte)(EVP_payload_length >> 0x10),
            ...
            for (int i = 1; i < EVP_LEGACY_HEADER_LEN; i++) {
        ...
        static int Transmission_Header_Decrypt(
            ...
            if (ssea::delta_decode(allocator, EVP_header_array, EVP_LEGACY_HEADER_LEN,
                APP->key.kf, decoded) != EVP_LEGACY_HEADER_LEN) {
            ...
            len = array[1] << 0x10 | array[2] << 0x08 | array[3];
        ...
        static std::shared_ptr<Byte> Transmission_Packet_Decrypt(
            ...
            if (datalen <= EVP_LEGACY_HEADER_LEN) {
                return ppp::diagnostics::SetLastError(ppp::diagnostics::ErrorCode::ProtocolFrameInvalid, NULLPTR);
            }
            ...
            int expected_len = payload_len + EVP_LEGACY_HEADER_LEN;
            ...
            memcpy(payload.get(), data + EVP_LEGACY_HEADER_LEN, payload_len);
        ...
        static std::shared_ptr<Byte> Transmission_Packet_Read(
            ...
            auto header = ITransmissionBridge::ReadBytes(transmission, y, EVP_LEGACY_HEADER_LEN);
            if (NULLPTR == header) {
                if (ppp::diagnostics::ErrorCode::Success == ppp::diagnostics::GetLastErrorCode()) {
                    ppp::diagnostics::SetLastErrorCode(ppp::diagnostics::ErrorCode::TunnelReadFailed);
                }
