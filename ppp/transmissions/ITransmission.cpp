...
#pragma GCC optimize("O1")
            ...
            static bool                                 Write(ITransmission* transmission, YieldContext& y, const void* packet, int packet_length) noexcept {
                ...
                if (NULL != py) {
                    ...
                    return ITransmissionBridge::Write(transmission, packet, packet_length, make_shared_object<AsynchronousWriteCallback>(
...
#pragma GCC pop_options
        ...
        private:
            ...
            static std::shared_ptr<Byte>                base94_encode(const std::shared_ptr<BufferswapAllocator>& allocator, Byte* data, int datalen, int& outlen) noexcept {
                ...
                memcpy(packet.get(), k.data(), EVP_HEADER_XSS);
                memcpy(packet.get() + EVP_HEADER_XSS, payload.get(), outlen);
        ...
        static std::shared_ptr<Byte>                    Transmission_Packet_Pack(
            ...
            memcpy(packet.get(), EVP_header.get(), EVP_header_length);
            memcpy(packet.get() + EVP_header_length, EVP_payload.get(), EVP_payload_length);
            return packet;
        }
        ...
        static std::shared_ptr<Byte>                    Transmission_Handshake_Pack_SessionId(
            ...
            if (max > 0) {
                ...
                int max_loops = RandomNext(min, max);
            ...
            memcpy(messages.get(), kfs, sizeof(kfs));
            memcpy(messages.get() + sizeof(kfs), packet, session_id_string.size());
            return messages;
        }
        ...
        static bool                                     Transmission_Handshake_Nop(
            ...
            for (int i = 0; i < roundof; i++) {
                bool b = Transmission_Handshake_SessionId(APP, transmission, y, 0);
                if (!b) {
                    return false;
                }
            }
        ...
        ITransmission::ITransmission(const ContextPtr& context, const AppConfigurationPtr& configuration) noexcept
        ...
        boost::asio::ip::tcp::endpoint ITransmission::GetRemoteEndPoint() noexcept {
            return boost::asio::ip::tcp::endpoint(boost::asio::ip::address_v4::broadcast(), ppp::net::IPEndPoint::MinPort);
        }

        ITransmission::AppConfigurationPtr ITransmission::GetConfiguration() noexcept {
        ...
        bool ITransmission::Write(const void* packet, int packet_length, const std::shared_ptr<AsynchronousWriteCallback>& cb) noexcept {
        ...
        ITransmission::ContextPtr ITransmission::GetContext() noexcept {
            return context_;
        }
