...
namespace ppp {
    namespace ethernet {
        ...
        int VNetstack::AllocVirtualPort() noexcept {
            for (int c = IPEndPoint::MinPort; c <= IPEndPoint::MaxPort; c++) {
                ...
                if (NULL == this->FindTcpLink(localPort)) {
                    return localPort;
                }
            }
        ...
        bool VNetstack::Constructor(bool lwip, const int& localPort) noexcept {
            ...
            acceptor->AcceptSocket = make_shared_object<SocketAcceptor::AcceptSocketEventHandler>(
        ...
        void VNetstack::ReleaseAllResources() noexcept {
            std::shared_ptr<SocketAcceptor> acceptor = std::move(acceptor_);
            if (NULL != acceptor) {
                acceptor_.reset();
                acceptor->Dispose();
            }

            Dictionary::ReleaseAllObjects(wan2lan_);
            Dictionary::ReleaseAllObjects(lan2wan_);
