...
namespace ppp {
    ...
    public:
        ...
        static bool             TryParse(const std::string& s, DateTime& dateTime) noexcept {
        ...
        static DateTime         Parse(const std::string& s) noexcept {
            DateTime dateTime;
            TryParse(s, dateTime);
            return dateTime;
        }
    ...
    public:
        std::string             ToString(const std::string& s) noexcept {
            std::string& sx = const_cast<std::string&>(s);
            return ToString(sx.data(), (int)sx.length());
        }
        std::string             ToString(const char* format) noexcept {
    ...
    public:
        ...
        DateTime                operator+(const DateTime& right) noexcept {
            int64_t ticks = const_cast<DateTime&>(right).Ticks();
